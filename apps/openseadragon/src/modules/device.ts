...
function toSnapshot(gamepad: Gamepad): GamepadSnapshot {
  const hasAxisInput = gamepad.axes.some(Boolean);
  const hasButtonInput = gamepad.buttons.some(btn => btn.pressed);
  return {
    status: (hasAxisInput || hasButtonInput) ? GamepadStatus.Active : GamepadStatus.Idle,
